...
> 2026-09-22 후속 적용: 현재 총354개(기존326+공산 경제정책10+후속 내정9+기술보급·계획행정9). 현재 구성·좌표·검증 상태는 [2차 확장 기록](HOK_KOREAN_SECOND_WAVE_PLAN.md)을 따른다. 아래326개 좌표·화면 관측은 이전 기준선 이력이며 후속 추가분의 게임 검증 결과가 아니다.

작성일: 2026-09-21

한국 중점 326개(기존 266개와 앞선 확장 60개)를 보존하면서 넓어진 배치를 압축했다. **현재 범위는 x0~194, y0~24이며 HIDE 초기 상태에서 전체 분야 화면 검수를 마쳤다.** 기간·효과·선행 AND/OR·상호배타·표시 조건·AI는 변경하지 않았다.
...
AA58과 현재 소스의 326개 focus 블록을 대조하여 좌표·상대 기준·offset 이동량과 주석을 제외한 토큰이 동일함을 확인했다. 통합 제안의 정적 중점 상자·연결선 기둥 모델은 간섭 후보를 보고하지 않았다. **실제 화면도 HIDE 초기 상태에서 전체 분야를 확인했으며, 관측 범위에서 이름판·아이콘 겹침과 관계없는 선의 중점 가림은 0건이었다.** 이 결과를 이념 전환 후 모든 표시 상태에 대한 검증으로 확대하지 않는다.
...
상대 ID가 “—”이면 절대 기준점이다. 나머지 행의 소스 x/y는 “상대 x/y”다. 현재 직접 offset이 모두 0이므로 조건부 이동량도 0이며, HIDE의 표시 여부와 구분한다. “확장”은 이번 작업 전에 존재하던 60개를 뜻한다.
