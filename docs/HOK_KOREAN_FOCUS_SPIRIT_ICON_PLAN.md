...
사용자는 앞으로 필요한 국민정신·중점 이미지를 **직접 새로 그리거나 생성하고, 또는 오픈소스·재사용이 허용된 공개 소재를 가져와 사용**하도록 방침을 정했다. 두 방법을 함께 써도 된다. 외부 소재 검색이나 기존 합성 스크립트 사용을 모든 새 그림의 필수 절차로 두지 않는다. 대상의 의미, 기존 그림과의 통일성, 작은 크기에서의 가독성, 작업량을 보고 이미지별로 선택한다.

| 제작 방법 | 적용 방식 | 남길 기록 |
|---|---|---|
| 직접 제작·AI 생성 | 정책에 맞는 중심 그림 또는 아이콘을 새로 그리거나 생성 | 제작 방식, 제작·생성 도구, 실제 확인 가능한 모델, 날짜, 생성 프롬프트와 참조 이미지, 채택 원본·최종 파일 경로와 해시 |
| 공개 소재 재사용 | 사용 조건이 허용하는 그림을 그대로 복사하거나 크기 조정·재색칠·합성 | 정확한 출처·파일·버전, 원저자·기여자, 적용되는 사용 조건, 원본 해시, 가공 내역 |
| 혼합 제작 | 새 중심 그림과 재사용 가능한 테두리·배경 등을 조합 | 직접 제작·생성 부분과 외부 소재 부분을 구분하여 각각의 제작·출처 기록 유지 |

- **승인된 콘텐츠 구현에 이미지 제작·연결을 포함한다.** 새 중점·국민정신 구현 범위 안에서는 위 방법을 선택하기 위해 별도의 이미지 요청을 다시 받을 필요가 없다. 문서화·설명·검토만 요청한 경우에는 이미지 생성·다운로드·가공이나 게임 적용을 시작하지 않는다.
- **공개되어 있다는 사실과 사용 허가는 구분한다.** 선택한 이미지에 적용되는 라이선스나 명시적 허용 범위 안에서 복사·변형·배포하고 크레딧을 보존한다. 저장소 코드의 라이선스를 그림 전체의 허가로 간주하지 않는다. 조건이 맞지 않으면 다른 허용 소재를 찾거나 새로 제작한다. 외부 이미지를 참고해 다시 그리거나 AI로 가공했더라도 해당 출처와 적용 조건을 없던 것으로 처리하지 않는다.
- **완성 이미지는 프로젝트 안에 둔다.** 채택한 생성 원본·재사용 원본을 프로젝트 내부 작업 경로에 보존하고, 게임용 결과는 `gfx/interface/goals/HOK_KOR/`·`gfx/interface/ideas/HOK_KOR/` 등 런타임 경로에 저장한다. 프로젝트 소유 GFX와 모드 루트 기준 상대경로로 연결하며 마스크·광택 오버레이도 내부 파일로 갖춘다. 생성 도구의 기본 출력 폴더나 외부 게임·모드 경로에만 있는 파일을 런타임에서 참조하지 않는다.
- **색상과 그림체 기준은 공통이다.** [색상 계획](HOK_KOREAN_FOCUS_ICON_COLOR_PLAN.md)과 [AGENTS.md](../AGENTS.md)의 정치 가지·분야별 색상을 적용한다. 중심 소재, 금속 테두리, 명암, 비율, 투명 여백을 맞추고 연결된 중점·국민정신은 같은 소재와 색상 계열을 공유한다. 실제 강화 단계만 `I`·`II`·`III`로 구분한다. 기존 완성품의 배경색만 바꾸는 작업은 중심 그림·금속 장식을 보존하는 방식으로 진행한다.
- **시안과 게임용 결과를 구분한다.** 현재 적용 아이콘은 중점 100×88, 국민정신 60×68의 알파 포함 BGRA32 DDS다. 새 이미지도 실제 대상 UI의 규격·형식에 맞춰 변환하고 작은 크기에서 검수한다. 생성된 PNG나 큰 미리보기만으로 적용 완료라고 하지 않는다. 일반·광택·단계 교체의 게임 검증 여부를 별도로 기록한다.
- **제작 이력을 정확히 남긴다.** AI 생성물을 공개 팩의 원화로 표시하지 않고, 외부 그림을 가져오거나 재색칠한 결과를 새로 그린 원화라고 표시하지 않는다. [크레딧](HOK_KOREAN_FOCUS_ICON_CREDITS.md)과 [매니페스트](data/HOK_KOREAN_FOCUS_ICON_MANIFEST.json)는 실제 채택·적용 시 갱신한다. 기존 60개·29개가 공개 소재 합성으로 제작됐다는 기록은 보존한다.

이번 변경은 제작 방침의 문서화다. 새 이미지의 채택·게임 적용이나 기존 아이콘 일괄 교체를 수행한 기록이 아니다. 채택·통합되지 않은 시안은 현재 자산으로 간주하지 않는다. 아래 최초 문서화 기록의 미승인·미실행 상태와 외부 소재 중심 절차는 당시 이력이며, 후속 제작에는 이 절과 현행 AGENTS.md의 규칙을 적용한다.
